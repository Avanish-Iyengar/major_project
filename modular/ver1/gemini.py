...
# Shared state updated by background thread, read by render loop
_report_state: dict = {
...
def _fetch_report_thread(prompt: str):
...
def _wrap_text(text: str, max_chars: int) -> list[str]:
    """Word-wrap preserving blank lines between paragraphs."""
    out = []
...
def show_report_window(prompt: str):
    """
    Opens a standalone OpenCV window with the Gemini coaching report.
    API call runs in a background thread — window appears instantly with a loader.
    Controls: UP/DOWN or W/S to scroll.  Q or ESC to close.
    """
    WIN   = "XAI — Gemini Coaching Report"
    WIN_W, WIN_H = 900, 700
    BG    = (18, 18, 28)
    HDR   = (0, 165, 255)
    TXT   = (220, 220, 230)
    DIM   = (90, 90, 110)
    SEC   = (100, 210, 140)   # section header colour
    ERR   = (60, 80, 220)
    ...
    # Reset shared state and start fetch
    _report_state["text"]  = "  Analyzing your session with Gemini..."
    _report_state["ready"] = False
    threading.Thread(target=_fetch_report_thread, args=(prompt,), daemon=True).start()
