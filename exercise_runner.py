...
""")
    ...
    while cap.isOpened():
        ...
        frame_idx += 1
        if frame_idx % 2 == 0:
            last_result = detect_pose(landmarker, frame)
        ...
        if result.pose_landmarks:
            ...
            if view in definition.valid_views:
                alerts, joint_angles, driver_angle = run_checks(definition, lms, pts, view)
            ...
            for jc in definition.joint_checks:
                if jc.display_name in joint_angles and jc.landmark_b in pts:
                    ...
                    cv2.putText(frame, f"{int(val)}", (int(pt[0]) + 10, int(pt[1]) - 10),
        ...
        draw_hud(frame, definition, state, fps)
